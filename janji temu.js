document.addEventListener('DOMContentLoaded', function () {
  const serviceDropdown = document.getElementById('poli');
  ...
  serviceDropdown.addEventListener('change', function () {
    ...
    if (selectedService && doctorsByService[selectedService]) {
      doctorsByService[selectedService].forEach(doctor_name => {
        const option = document.createElement('option');
        option.value = doctor_name;
        option.textContent = doctor_name;
  ...
  form.addEventListener('submit', function (event) {
    ...
    console.log('Form Data:', [...formData.entries()]); // Debug: lihat data yang dikirim
    ...
    fetch('submit_appointment.php', {
      ...
      .then(data => {
        console.log('Server Response:', data); // Debug: lihat respons dari server
        ...
        if (data.status === 'success') {
          ...
          alert(data.message); // Menampilkan pesan error jika status bukan success
        }
      })
      .catch(error => {
